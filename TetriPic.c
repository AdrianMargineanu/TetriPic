...
//Functia indeplineste task 1
void TASK_1() {
FILE* fout;

  fout = fopen("piesa_I.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'I', 0, 4, 1);
  fclose(fout);

  fout = fopen("piesa_O.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'O', 0, 2, 2);
  fclose(fout);

  fout = fopen("piesa_S.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'S', 0, 2, 3);
  fclose(fout);

  fout = fopen("piesa_Z.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'Z', 0, 2, 3);
  fclose(fout);

  fout = fopen("piesa_L.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'L', 0, 3, 2);
  fclose(fout);

  fout = fopen("piesa_J.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'J', 0, 3, 2);
  fclose(fout);

  fout = fopen("piesa_T.bmp", "wb");
  CREATE_IMAGE_PIECE(fout, 'T', 0, 2, 3);
  fclose(fout);
}
//Functia indeplineste task 2
void TASK_2() {
  FILE* fout;
...
//FUnctia indeplineste task 3
void TASK_3() {
  int i, j, map_h, map_w, moves;
  infopixel** map;
  FILE* fin,*fout;
  fin = fopen("cerinta3.in", "r");
...
void TASK_4() {
  ...
  fin = fopen("cerinta4.in", "r");
  fscanf(fin, "%d", &moves);
  GAME(fin, map, moves, map_h, map_w);

  fclose(fin);
  fout = fopen("task4.bmp", "wb");
  WRITE_HEADER(map_w, map_h, fout);
  WRITE_PIECE(fout, map, map_h, map_w);
  fclose(fout);
  for (i = 0; i < map_h ; i++){
